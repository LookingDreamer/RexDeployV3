...
sub sync {
  ...
  Rex::Logger::info("[文件传输] cmd: $cmd");

  eval {
    my $return_success = 'false';
    ...
    eval {
      ...
      exp_continue;
        ...
        [
          qr{return_result0},
          sub {
            Rex::Logger::info("[文件传输] rsync返回码为真,文件传输成功");
            $return_success = 'true';
            die;
          }
      ...
      if ( $return_success eq 'false' ) {
        Rex::Logger::info("[文件传输] rsync返回码为假,同步文件异常,请配置调试模式查看异常原因",'error');
        die;
      }
      ...
      $exp->expect(
        ...
        [
          ...
          sub {
            Rex::Logger::info("传输成功");
            Rex::Logger::info("Finished transfer very fast");
      ...
      $exp->expect(
        ...
        [
          ...
          sub {
            Rex::Logger::info("传输成功");
            Rex::Logger::info("Finished transfer");
