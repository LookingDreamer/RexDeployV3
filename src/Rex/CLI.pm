...
sub _list_tasks {
  ...
  Rex::Config->register_config_handler("$env", sub {
   my ($param) = @_;
   $list_all_task_status = $param->{list_all_task_status} ;
   $list_task_string = $param->{list_task} ;
   @list_task = split(/,/,$list_task_string);
  });
  
  if($list_all_task_status eq "true"){
    @list_task = @tasks;
  }

  for my $task (@list_task) {
