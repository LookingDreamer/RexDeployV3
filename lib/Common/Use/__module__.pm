...
Rex::Config->register_config_handler(
    ...
    sub {
        ...
        $is_download_process_status = $param->{is_download_process_status};
    }
);
...
task "download", sub {
  ...
  if (Rex::is_sudo && "$http" ne "1") {
   ...
   my $time_start=time();
   my $downres = download_thread($dir1,$dir2,$http,$w,$is_download_process_status,$rsyncdelete);
   my $time_end=time();
...
sub download_thread{
my ($dir1,$dir2,$http,$w,$is_download_process_status,$rsyncdelete) = @_;
my %data ;
my $src = $dir1 ;
$dir1=~ s/ //g;
my $server = Rex::get_current_connection()->{server};
...
if ( "$download_method" eq "rsync" ) {
    ...
    if ( is_dir($dir1) ) {
      ...
      my $mainProcess = $$;
      my $download_random_file = "/tmp/download_".$mainProcess."_".get_random( 8, 'a' .. 'z' ).".txt";
      if ( "$is_download_process_status" eq "1") {
         download_process($zipFileName,$dir2,$download_random_file,$server,$filename);
      }      
      download "$zipFileName", "$dir2/$filename";
      Rex::Logger::info("$zipFileName -> $dir2/$filename下载完成");
      if ( "$is_download_process_status" eq "1") {
          LOCAL{
              run "touch $download_random_file";
          };
      }     
      unlink($zipFileName);
      ...
      my $download_random_file = "/tmp/download_".$mainProcess."_".get_random( 8, 'a' .. 'z' ).".txt";
      if ( "$is_download_process_status" eq "1") {
         download_process($dir1,$dir2,$download_random_file,$server,$filename);
      }  
...
sub download_process{
    my ($dir1,$dir2,$download_random_file,$server,$filename) = @_;    
    my %hash_pids;
    my $dir1md5 = md5("$dir1");
    my $dir1size = run "du -s $dir1  |awk '{print \$1}'";
    Rex::Logger::info("文件: $dir1 大小: $dir1size md5值: $dir1md5");
    my $child=fork(); #派生一个子进程
    if($child){   # child >; 0, so we're the parent
        $hash_pids{$child} = $child;  
        Rex::Logger::info("下载父进程PID:$$ 下载时进度条子进程PID:$child");
    }else{
        #在子进程中执行相关动作开始
        LOCAL{
            for (my $var = 1; $var <= 10 ; $var++) {
                select(undef, undef, undef, 1);
                if ( is_file("$download_random_file") ) {
                    Rex::Logger::info("[$server] 下载:$dir2/$filename - 100% OK!\n");
                    Rex::Logger::info("$dir1=>$dir2/$filename 检测到下载已经完成,退出");
                    unlink("$download_random_file");
                    select(undef, undef, undef, 2);
                    exit;
                }
                if ( is_file("$dir2/$filename") ) {
                  my $dir2size = run "du -s $dir2/$filename  |awk '{print \$1}'";
                  my $dir2md5 = md5("$dir2/$filename");
                  my $percent = int($dir2size/$dir1size*100);
                  Rex::Logger::info("[$server] 下载:$dir2/$filename - $percent% OK!\n");
                  # Rex::Logger::info("$dir1=>$dir2/$filename 本地文件当前大小: $dir2size 本地文件当前md5值: $dir2md5");
                  if ( "$dir2md5" eq  "$dir1md5") {
                      Rex::Logger::info("$dir1=>$dir2/$filename md5值相等,下载完成,退出");
                      Rex::Logger::info("[$server] 下载:$dir2/$filename - 100% OK!\n");
                      unlink("$download_random_file");
                      select(undef, undef, undef, 2);
                      exit;
...
if ( "$upload_method" eq "rsync" ) {
    ...
    my $filename = basename($zipFileName);
    my $mainProcess = $$;
    my $download_random_file = "/tmp/upload_".$mainProcess."_".get_random( 8, 'a' .. 'z' ).".txt";
...
sub upload_process{
    ...
    my $dir1size;
    LOCAL {
        $dir1md5 = run "md5sum $dir1  |awk '{print \$1}'";
        $dir1size = run "du -s $dir1  |awk '{print \$1}'";      
    };
    Rex::Logger::info("文件: $dir1 大小: $dir1size md5值: $dir1md5");
    my $child=fork(); #派生一个子进程
    if($child){   # child >; 0, so we're the parent
        $hash_pids{$child} = $child;  
        Rex::Logger::info("上传父进程PID:$$ 上传时进度条子进程PID:$child");
    }else{
        #在子进程中执行相关动作开始
        for (my $var = 1; $var <= 10 ; $var++) { 
            select(undef, undef, undef, 0.5);      
            run_task "Common:Use:uploadDeal",on=>$server,params => { 
              dir1 => "$dir1",
              dir2 => "$dir2",
              download_random_file => "$download_random_file",
              server => "$server",
              filename => "$filename",
              dir1md5 => "$dir1md5",
              dir1size => "$dir1size"
...
task uploadDeal =>,sub {
    ...
    my $dir1 = $self->{dir1};
    my $dir2 = $self->{dir2};
    my $download_random_file = $self->{download_random_file};
    my $server = $self->{server};
    my $filename = $self->{filename};
    my $dir1md5 = $self->{dir1md5};
    my $dir1size = $self->{dir1size};
    if ( is_file("$download_random_file") ) {
        Rex::Logger::info("[$server] 上传:$dir1 - 100% OK!\n");
        Rex::Logger::info("$dir1=>$dir2/$filename 检测到上传已经完成,退出");
        unlink("$download_random_file");
        select(undef, undef, undef, 2);
        exit;
    }
    ...
    if ( is_file("$dir2/$filename") ) {
      ...
      my $dir2md5 = md5("$dir2/$filename");
      my $percent = int($dir2size/$dir1size*100);
      Rex::Logger::info("[$server] 上传:$dir1 - $percent% OK!\n");
      # Rex::Logger::info("$dir1=>$dir2/$filename 本地文件当前大小: $dir2size 本地文件当前md5值: $dir2md5");
      if ( "$dir2md5" eq  "$dir1md5") {
          Rex::Logger::info("$dir1=>$dir2/$filename md5值相等,上传完成,退出");
          Rex::Logger::info("[$server] 上传:$dir1 - 100% OK!\n");
          unlink("$download_random_file");
          select(undef, undef, undef, 2);
          exit;
...
task json =>,sub {
  ...
  if ( "$w" eq "1" && "$random" eq "" ) {
    ...
    open OUT,"> $hashFile" or die "print $!\n";
...
desc "同步文件: rex Common:Use:syncFiles --files='source:dest'";
task "syncFiles", sub {
    my $self = shift;
    my $files = $self->{files};
    my $w = $self->{w};
    ...
    if( $filesCount  == 0 ){
        $data{"code"} = -1 ;
        $data{"msg"} = "files格式不正确" ;
        Rex::Logger::info("files格式不正确: --files='source:dest'","error");
        Common::Use::json($w,"","files格式不正确",[\%data],$random);
        return \%data;
    }
    for my $files (@filesArray  ){
        my @pathArray = split(/:/,$files);
        my $pathCount = @pathArray;
        if( $pathCount != 2 ){
            $data{"code"} = -1 ;
            Rex::Logger::info("解析files: $files 格式不正确: --files='source:dest'","error");
            Common::Use::json($w,"","解析files: $files 格式不正确",[\%data],$random );
            $data{"msg"} = "解析files: $files 格式不正确";
            return \%data;
        }
        my $source = $pathArray[0] ;
        my $dest = $pathArray[1] ;
        LOCAL{
            if(!is_file($source)){
                $data{"code"} = -1 ;
                Rex::Logger::info("本地文件:$source 不存在","error");
                Common::Use::json($w,"","本地文件:$source 不存在",[\%data],$random );
                $data{"msg"} = "本地文件:$source 不存在";
            }
        };
        if( $data{"code"} == -1){
            return \%data;
        }
        if(  is_writable("$dest") == 0  ) {
            $data{"code"} = -1 ;
            Rex::Logger::info("远程文件:$dest 不可以写,请确认是否正确的配置了远程文件","error");
            Common::Use::json($w,"","远程文件:$dest 不可以写,请确认是否正确的配置了远程文件",[\%data],$random );
            $data{"msg"} = "远程文件:$dest 不可以写,请确认是否正确的配置了远程文件";
            return \%data;
        }
        if(  is_dir("$dest") ) {
            $data{"code"} = -1 ;
            Rex::Logger::info("远程文件:$dest 是目录,请确认是否正确的配置了远程文件","error");
            Common::Use::json($w,"","远程文件:$dest 是目录,请确认是否正确的配置了远程文件",[\%data],$random );
            $data{"msg"} = "远程文件:$dest 是目录,请确认是否正确的配置了远程文件";
            return \%data;
        }

    }
    Rex::Logger::info("操作主机:$server");
    my @changeFile ;
    my @nochangeFile ;
    for my $files (@filesArray  ){
        my @pathArray = split(/:/,$files);
        my $source = $pathArray[0] ;
        my $dest = $pathArray[1] ;
        Rex::Logger::info("本地文件:$source 远程文件:$dest");
        my $result = file "$dest",
                     source => "$source",
                     on_change => sub { push @changeFile,$dest; };
        if( $result  == 0 ){
            push @nochangeFile,$dest;
        }
    }
    $data{"changeFile"} = \@changeFile;
    $data{"nochangeFile"} = \@nochangeFile;
    my $changeFileVar = join(",",@changeFile);
    my $nochangeFileVar = join(",",@nochangeFile);
    if($changeFileVar){
        Rex::Logger::info("以下远程文件发生了变化:$changeFileVar");
    }elsif($nochangeFileVar){
        Rex::Logger::info("以下远程文件没有变化(md5值相等):$nochangeFileVar");
    }else{
        $data{"code"} = -1 ;
        Rex::Logger::info("同步异常","error");
        Common::Use::json($w,"","同步异常",[\%data],$random );
        $data{"msg"} = "同步异常";
        return \%data;         
    }
    Common::Use::json($w,"0","同步成功",[\%data],$random);
    return \%data; 
};
